...
def load_data(train_path, val_path, im_size=HEIGHT*WIDTH):
  ...
  group_classes = (len(os.listdir(val_path)) == 2) and (len(os.listdir(train_path)) == 5) # True if only two classes on validation set, but 5 on training set
...
def svm_classifier(x_train, y_train, x_val, y_val, gamma=0.0078125, C=32768, class_weight=None, name=""):

  # create classifier
  classifier = svm.SVC(gamma=gamma, C=C, class_weight=class_weight)

  print(time.time(), "Training SVM classifier")
  # train classifier
  classifier.fit(x_train, y_train)
  joblib.dump(classifier, name+'_svm_model.pkl')
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Classify images using SVM')
  ...
  parser.add_argument('--points', dest='points', type=int, action='store', default=24)
  parser.add_argument('--glcm', action='store_true')
  parser.add_argument('--angles', dest='angles', nargs='+', action='store', default=[1,5,8])
  parser.add_argument('--distances', dest='distances', nargs='+', action='store', default=[0, math.pi/2, 3*math.pi/4])
  parser.add_argument('--gamma', dest='gamma', action='store', type=float, default=0.0078125)
  parser.add_argument('--C', dest='C', action='store', type=float, default=32768)
  parser.add_argument('--class_weight', dest='class_weight', nargs='+', action='store', default=None)
  parser.add_argument('--name', dest="name", action='store', default="")
  args = parser.parse_args()

  if len(args.class_weight) == 1:
    class_weight = args.class_weight[0]
  elif (len(class_weight) == 5) and (len(os.listdir(args.val_path)) == 2):
